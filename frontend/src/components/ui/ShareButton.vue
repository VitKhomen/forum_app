<template>
  <div class="relative" ref="triggerRef">
    ...
    <!-- Teleport в body щоб уникнути overflow-hidden батьків -->
    <Teleport to="body">
...
// Позиція дропдауну в координатах вікна
...
// Рахуємо позицію відносно кнопки
const updatePosition = async () => {
  ...
  // По горизонталі — вирівнюємо вправо від кнопки, але не виходимо за край
  let left = rect.right - dropWidth
  if (left < margin) left = margin
  if (left + dropWidth > window.innerWidth - margin) {
    left = window.innerWidth - dropWidth - margin
  }

  // По вертикалі — спочатку намагаємось вниз, якщо не влізе — вгору
  const dropHeight   = hasNativeShare ? 172 : 132
  const spaceBelow   = window.innerHeight - rect.bottom
  const top = spaceBelow >= dropHeight + margin
    ? rect.bottom + margin
    : rect.top - dropHeight - margin
...
// Перераховуємо при скролі та ресайзі
...
const copyLink = async () => {
  ...
  copied.value = true
  setTimeout(() => { copied.value = false; open.value = false }, 1800)
}
...
const nativeShare = async () => {
  try {
    await navigator.share({ title: props.title, text: props.text || props.title, url: shareUrl.value })
  } catch (e) {
